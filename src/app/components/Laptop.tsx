...
type LaptopProps = {
  videoSrc?: string;
  className?: string;
  loaded?: boolean; // ajoute/retire la classe "loaded"
  opened?: boolean; // ajoute/retire la classe "opened"
  /** Facteur d’échelle (1 = taille actuelle, 0.8 = 80%, etc.) */
  scale?: number;
};

export default function Laptop({
  videoSrc = "https://d1xm195wioio0k.cloudfront.net/images/video/support.mp4",
  className = "",
  loaded = true,
  opened = true,
  scale = 1, // ⬅️ utilise ceci pour réduire/agrandir
}: LaptopProps) {
  return (
    <div className={`container ${className}`}>
      <div
        className={[
          ...
          .join(" ")
          .trim()}
        // ⬇️ on scale tout le mockup, ancré en bas-centre
        style={{
